<?php
class ModelShippingFreteclick extends Model {
	...
	private function getProducts() {
		...
		foreach ($produtos as $key => $prod) {
			$items[$key] = [
				...
				'height' => number_format($this->getDimensaoEmCm($prod['length_class_id'], $prod['height'] / 100), 2, ',', ''),
				'width' => number_format($this->getDimensaoEmCm($prod['length_class_id'], $prod['width'] / 100), 2, ',', ''),
				'depth' => number_format($this->getDimensaoEmCm($prod['length_class_id'], $prod['length'] / 100), 2, ',', ''),
			];
		}
		// print_r($items); exit;
		return $items;
