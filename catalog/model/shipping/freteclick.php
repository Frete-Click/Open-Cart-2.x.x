...
class ModelShippingFreteclick extends Model {
	function getQuote($address) {
		...
		if (!$this->config->get('freteclick_geo_zone_id')) {
			$status = true;
		...
		if ($this->cart->getSubTotal() < $this->config->get('freteclick_total')) {
			$status = false;
		}
		...
		if ($status) {
			...
			$quote_data['freteclick'] = array(
				'code'         => 'freteclick.freteclick',
